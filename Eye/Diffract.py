...
class Human(Report):
    ...
    aawf = {'radius'    : 0e+0,
            ...
            'wavelength': 4e-7,
            'focal'     :17e-3}
    u0   = 3.831705970207512    # First zeropoint (by hand in gnuplot)
    zeroPoints = [ # u values where Airy goes to zero, discovered by hand.
            # Suspect: First zero might be at u=3.831705970207512"
            3.83170611001,      # First zeropoint (by algorithm in scipy)
            7.01558711101,
    ...
    def circle(self, **kw):
        ...
        r = (u0*w*f)/(pi*a)# if r != 0.0 else 0.0
        return r

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    def wave(self, **kw):
        "First zero is at u=3.831705970207512"
        amplitude, r, a, w, f = (
                kw.get(key, Human.aawf[key]) for key in (
                    'amplitude', 'radius', 'aperture', 'wavelength', 'focal'))
        u = (pi*r*a)/(w*f)
